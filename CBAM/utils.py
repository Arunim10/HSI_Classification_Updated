...
def spatial_avg(patch,coordinates,kernel_size=3):
    (x,y,c) = coordinates
    data = patch[:,:,c].copy()
    while (np.isnan(data[x,y]) and kernel_size<20):
...
def f_score(y_true,y_pred):
    ...
    scores = f1_score(y_true, y_pred, average=None,labels=[i for i in range(76)])
    return scores
    ...
    for epoch in range(epochs):
        ...
        if test_dl is not None:
            ...
            if (np.mean(f1_val)>best_eval_score):
                ...
                torch.save(model.state_dict(), 'D:\\HSI Project\\Updated_Work\\HSI_Classification\\CBAM\\cbam_model3.pth')
