...
def train_model(model,train_dl,test_dl,epochs,device,lr,wd):
    ...
    # conv_params = {'params': [param for name, param in model.named_parameters() if 'weight' in name],
    #             'lr': 1, 'weight_decay': 1}
    # bias_params = {'params': [param for name, param in model.named_parameters() if 'bias' in name],
    #             'lr': 2, 'weight_decay': 0}

    # Applied above but I am getting nan values for loss, so its not giving good results

    # """

    # optim = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9,weight_decay=wd)
    # scheduler = StepLR(optim, step_size=epochs//3, gamma=0.1)
