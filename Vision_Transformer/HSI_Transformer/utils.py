    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_dl,desc='Train',mininterval=30) as train_progress:
            for i,(spec_values,labels) in enumerate(train_progress):
                    ...
                    label_pred = model(spec_values)
                    label_pred = label_pred.squeeze()
                    labels = labels.squeeze()
                    # print(label_pred.shape," ",labels.shape)
                    loss = torch.nn.functional.cross_entropy(label_pred,labels)
                    # print(loss)
                    # print(torch.min(label_pred), torch.max(label_pred))
                    ...
                    if (np.isinf(loss.item()) or np.isnan(loss.item())):
                        ...
                        nan_mask = torch.isnan(spec_values)
                    ...
                    train_progress.set_description(f'Train: loss:{loss:.4f},lr:{lr:.4f},f1:{np.mean(f1_temp):.4f},acc:{acc_temp:.4f},gpu_mem={mem:.2f} GB')
        ...
        if test_dl is not None:
            ...
            if (np.mean(f1_val)>best_eval_score):
                ...
                torch.save(model.state_dict(), 'D:\\HSI Project\\Updated_Work\\HSI_Classification\\Vision_Transformer\\HSI_Transformer\\MViT_Model_1.pth')
